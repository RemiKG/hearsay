...
// The record dir is overridable; on a serverless host (Vercel) it falls back to the
// only writable dir, /tmp. The Docker/ECS production target uses the repo default.
const DIR = process.env.HEARSAY_DATA_DIR || (process.env.VERCEL ? '/tmp/hearsay-records' : path.join(ROOT, '.data', 'records'));
